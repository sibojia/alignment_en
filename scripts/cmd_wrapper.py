...
def htk(filein, fileout, wavfile):
	...
	f.write(wavfile)
	f.close()
	s=r"..\htk\HVite.exe -A -D -T 1 -l '*' -C ..\htk\models\config.fromwav -H ..\htk\models\wsj_mono\macros -H ..\htk\models\wsj_mono\hmmdefs -i %s -m -y lab -t 1000 20000 41000 -a -I %s -S train.scp ..\htk\models\monophone_sp_ext.dict ..\htk\models\monophones_sp" % (fileout, filein)
	tic = time.time()
